...
func (uc *usecase) SaveAddresses(ctx context.Context, req *participantdto.SaveAddressesRequest) ([]participantdto.AddressResponse, error) {
	if len(req.Addresses) == 0 {
		return nil, errors.ErrBadRequest("addresses must contain at least one item")
	}

	var result []participantdto.AddressResponse
	...
	err := uc.txManager.WithTransaction(ctx, func(txCtx context.Context) error {
		...
		result = make([]participantdto.AddressResponse, 0, len(insertedAddresses))
