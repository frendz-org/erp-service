...
func mapIdentityToResponse(identity *entity.ParticipantIdentity) IdentityResponse {
...
func mapAddressToResponse(address *entity.ParticipantAddress) AddressResponse {
...
func mapBankAccountToResponse(account *entity.ParticipantBankAccount) BankAccountResponse {
...
func mapFamilyMemberToResponse(member *entity.ParticipantFamilyMember) FamilyMemberResponse {
...
func mapEmploymentToResponse(employment *entity.ParticipantEmployment) EmploymentResponse {
...
func mapPensionToResponse(pension *entity.ParticipantPension) PensionResponse {
...
func mapBeneficiaryToResponse(beneficiary *entity.ParticipantBeneficiary) BeneficiaryResponse {
...
func mapStatusHistoryToResponse(history *entity.ParticipantStatusHistory) StatusHistoryResponse {
...
func BuildStepsCompleted(m map[string]bool) StepsCompleted {
	if m == nil {
...
func (uc *usecase) buildFullParticipantResponse(ctx context.Context, participant *entity.Participant, concurrent bool) (*ParticipantResponse, error) {
	...
	resp.Identities = make([]IdentityResponse, 0, len(identities))
	for _, identity := range identities {
		resp.Identities = append(resp.Identities, mapIdentityToResponse(identity))
	}

	resp.Addresses = make([]AddressResponse, 0, len(addresses))
	for _, address := range addresses {
		resp.Addresses = append(resp.Addresses, mapAddressToResponse(address))
	}

	resp.BankAccounts = make([]BankAccountResponse, 0, len(bankAccounts))
	for _, account := range bankAccounts {
		resp.BankAccounts = append(resp.BankAccounts, mapBankAccountToResponse(account))
	}

	resp.FamilyMembers = make([]FamilyMemberResponse, 0, len(familyMembers))
	...
	resp.Beneficiaries = make([]BeneficiaryResponse, 0, len(beneficiaries))
