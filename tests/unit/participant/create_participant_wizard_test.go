...
func TestCreateParticipant_DraftExists_ReturnsDraftExistsError(t *testing.T) {
	...
	uc, txMgr, participantRepo, _, _, _, _, _, _, _, _ := makeCreateUsecase()
	...
	req := &participant.CreateParticipantRequest{
		TenantID:       tenantID,
		ProductID:      productID,
		UserID:         userID,
...
func TestCreateParticipant_Success_StoresKTPAndEmployee(t *testing.T) {
	...
	uc, txMgr, participantRepo, statusHistoryRepo, identityRepo, addressRepo, bankAccountRepo, familyMemberRepo, employmentRepo, pensionRepo, beneficiaryRepo := makeCreateUsecase()
	...
	identityRepo.On("ListByParticipantID", mock.Anything, mock.AnythingOfType("uuid.UUID")).
		Return([]*entity.ParticipantIdentity{}, nil)
	addressRepo.On("ListByParticipantID", mock.Anything, mock.AnythingOfType("uuid.UUID")).
		Return([]*entity.ParticipantAddress{}, nil)
	bankAccountRepo.On("ListByParticipantID", mock.Anything, mock.AnythingOfType("uuid.UUID")).
		Return([]*entity.ParticipantBankAccount{}, nil)
	familyMemberRepo.On("ListByParticipantID", mock.Anything, mock.AnythingOfType("uuid.UUID")).
		Return([]*entity.ParticipantFamilyMember{}, nil)
	employmentRepo.On("GetByParticipantID", mock.Anything, mock.AnythingOfType("uuid.UUID")).
		Return(nil, errors.ErrNotFound("not found"))
	pensionRepo.On("GetByParticipantID", mock.Anything, mock.AnythingOfType("uuid.UUID")).
		Return(nil, errors.ErrNotFound("not found"))
	beneficiaryRepo.On("ListByParticipantID", mock.Anything, mock.AnythingOfType("uuid.UUID")).
