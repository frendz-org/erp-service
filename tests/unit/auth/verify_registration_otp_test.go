...
func TestVerifyRegistrationOTP(t *testing.T) {
	...
	tests := []struct {
		name          string
		req           *auth.VerifyRegistrationOTPRequest
	...
	}{
		{
			...
			req:  &auth.VerifyRegistrationOTPRequest{Email: email, OTPCode: validOTP},
		...
		},
		{
			...
			req:  &auth.VerifyRegistrationOTPRequest{Email: "wrong@example.com", OTPCode: validOTP},
			setupMocks: func(redis *MockInMemoryStore) {
				session := &entity.RegistrationSession{
					ID:           registrationID,
					Email:        email,
		...
		},
		{
			...
			req:  &auth.VerifyRegistrationOTPRequest{Email: email, OTPCode: "000000"},
			setupMocks: func(redis *MockInMemoryStore) {
				session := &entity.RegistrationSession{
					ID:           registrationID,
					Email:        email,
