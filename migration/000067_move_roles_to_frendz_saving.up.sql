...
BEGIN
    ...
    UPDATE permissions SET product_id = v_frendz_product_id, updated_at = NOW()
    WHERE product_id = v_iam_product_id AND (code LIKE 'participant:%' OR code LIKE 'member:%') AND deleted_at IS NULL;

    UPDATE roles SET product_id = v_frendz_product_id, updated_at = NOW()
    WHERE product_id = v_iam_product_id AND code IN ('PARTICIPANT_CREATOR', 'PARTICIPANT_APPROVER', 'TENANT_PRODUCT_ADMIN') AND deleted_at IS NULL;

    RAISE NOTICE 'Moved participant/member roles and permissions to frendz-saving';
END $$;
